...
import { accoutRecomendation, obtainPostsFromUserExecutedBy, patchTwit } from "./request_module";
...
describe('Recomendation Tests', () => {
    ...
    it ("The recommended user for user 2 should be 1", async () => {
        mAxios.get.mockResolvedValueOnce({data:[]}).mockResolvedValueOnce({data:{interests:[]}}).mockResolvedValueOnce({data:{users:[{uid:10}]}}).mockResolvedValueOnce({data:{interests:["string"]}}).mockResolvedValueOnce({data:[]});
        let recomendation_response = await accoutRecomendation("2");
        expect(recomendation_response.status).toBe(200);
        expect(recomendation_response.body[0].id).toBe("1")
    })
